...
# Cargar datos con encabezado
train_sano = np.genfromtxt("Data_ACV_NO_70.csv", delimiter=",", skip_header=1)
train_malo = np.genfromtxt("Data_ACV_SI_70.csv", delimiter=",", skip_header=1)

Dt = 142
X_train = np.vstack((train_sano[:Dt, :7], train_malo[:Dt, :7]))
...
test_sano = np.genfromtxt("Data_ACV_NO_30.csv", delimiter=",", skip_header=1)[:, :7]
test_malo = np.genfromtxt("Data_ACV_SI_30.csv", delimiter=",", skip_header=1)[:, :7]
...
def predecir():
    try:
        entrada = np.array([
            ...
            float(age_var.get()),
            float(hypertension_var.get()),
            float(heart_disease_var.get()),
