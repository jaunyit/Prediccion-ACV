...
def obtener_datos():
    try:
        ...
        ht = cond_map[ht_var.get()]
        hd = cond_map[hd_var.get()]
        smoke = smoke_map[fuma_var.get()]
        age = float(edad_var.get())
        gluc = float(glucosa_var.get())
        bmi = float(bmi_var.get())
        heart_hyperten = round((ht + hd) / 2, 1)
        return [gender, age, heart_hyperten, gluc, bmi, smoke]
...
titulo = tk.Label(
    ...
    font=("Arial", 20, "bold"),
    fg="#212594",  # Azul marino
    bg="#6facd9"   # Asegúrate que coincida con el fondo
)
...
ventana.geometry("600x670")  # Tamaño inicial
...
def add_dropdown(label, var, opciones):
    ...
    # También puedes cambiar el menú desplegable:
...
def add_entry(label, var):
    tk.Label(ventana, text=label, font=fuente_label, bg="#6facd9").pack(anchor="w", padx=10)
...
tk.Button(ventana, text="Predecir con SVM", command=predecir_svm, font=fuente_label).pack(pady=4)
tk.Button(ventana, text="Predecir con Bayesiano", command=predecir_bayes, font=fuente_label).pack(pady=4)
tk.Button(ventana, text="Predecir con Red Neuronal", command=predecir_red, font=fuente_label).pack(pady=4)
