...
from tkinter import (
    Tk, Label, Entry, Button, DoubleVar, StringVar,
...
def leer_y_limpiar(ruta_csv: str) -> pd.DataFrame:
    ...
    df[["hypertension", "heart_disease"]] = df[["hypertension", "heart_disease"]].apply(pd.to_numeric, errors="coerce")
    df["heart_hyperten"] = ((df["hypertension"].fillna(2) + df["heart_disease"].fillna(2)) / 2).round(1)

    cols_finales = ["gender", "age", "heart_hyperten",
                    "avg_glucose_level", "bmi", "smoking_status", "stroke"]
    df = df[cols_finales]

    df[df.columns.drop("stroke")] = df.drop(columns="stroke").apply(pd.to_numeric, errors="coerce")
    imp = SimpleImputer(strategy="median")
    df[df.columns.drop("stroke")] = pd.DataFrame(
        imp.fit_transform(df.drop(columns="stroke")),
        index=df.index,
        columns=df.columns.drop("stroke")
    )
...
class StrokeGUI:
    ...
    def predecir(self):
        ...
        heart_hyperten = round((ht_code + hd_code) / 2, 1)
        ...
        columnas = ["gender", "age", "heart_hyperten",
